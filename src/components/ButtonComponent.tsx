    ...
    return type === '#129575' ? (
        <TouchableOpacity 
            ...
            style = {[globalStyle.button, {
                backgroundColor: color ? color : disable ? COLORS.HEX_LIGHT_GREY : COLORS.TEAL_GREEN,
            }, styles]}>
            {icon && iconFlex === 'left' && icon}
            <TextComponent 
                text={text}
                color={textColor ?? COLORS.WHITE}
                font={textFont ?? FONTFAMILY.montserrat_bold}
                styles= {[textStyles, {
        ...
        <TouchableOpacity onPress={onPress}>
            <TextComponent 
                text={text}
                color={type === 'link' ? COLORS.ORANGE_PEEL : COLORS.BLACK}/>
        </TouchableOpacity>
    )
}
